...
//        int[] nums = {1,2,4,8};
        System.out.println(new Question368().largestDivisibleSubset(nums));
    }
    ...
    public List<Integer> largestDivisibleSubset(int[] nums) {
        ...
        int maxLength = 1;
        // 当前最长整除子集的开头元素
        int maxStartVal = maxValue;

        // 遍历各元素
        for (int j = nums.length-1; j >= 0; j--) {
            int num = nums[j];
            boolean hasAdd = false; // 是否找到可以下一元素
            int nextVal = 0;    // 当前元素的下一元素
            int currMax = 0;    // 当前元素的最大整除子集长度

            // 判断用哪种方式进行 dp
            if (maxValue/num < nums.length-j) {
                // 以当前元素的倍数为基准进行遍历
                for (int i = num * 2; i <= maxValue; i += num) {
                    if (hashMap.containsKey(i)) {
                        int size = hashMap.get(i).size() + 1;
                        if (size > currMax) {
                            hasAdd = true;
                            nextVal = i;
                            currMax = size;
            ...
            if (hasAdd) {
                ...
                list.add(num);
                list.addAll(hashMap.get(nextVal));
        ...
        return hashMap.get(maxStartVal);
    }
}
